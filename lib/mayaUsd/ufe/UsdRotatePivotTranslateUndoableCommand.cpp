...
void UsdRotatePivotTranslateUndoableCommand::undo()
{
	...
	const GfVec3f newRotatePivot = fPrevPivotValue;
	const GfVec3f oldRotatePivot = api.rotatePivot(fTimeCode);
	if(!GfIsClose(newRotatePivot, oldRotatePivot, 1e-5f))
	{
		api.rotatePivot(newRotatePivot, fTimeCode);
	}
...
bool UsdRotatePivotTranslateUndoableCommand::translate(double x, double y, double z)
{
	...
	MayaUsdUtils::MayaTransformAPI api(fPrim);
	const GfVec3f newRotatePivot = GfVec3f(x, y, z);
	const GfVec3f oldRotatePivot = api.rotatePivot(fTimeCode);
	if(!GfIsClose(newRotatePivot, oldRotatePivot, 1e-5f))
	{
		api.rotatePivot(newRotatePivot, fTimeCode);
