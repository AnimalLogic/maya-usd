...
//----------------------------------------------------------------------------------------------------------------------
/// \brief  When importing a new usd stage into an AL::usdmaya::nodes::ProxyShape, there are a selection of post import
///         processes that need to take place in order to import any custom usd plugins found.
///
///         \li If any custom plugin prims are found, that represent DAG nodes (e.g. they are shapes or transforms),
///             then a hierarchy of AL::usdmaya::nodes::Transform nodes will be generated so that the USD transform
///             values found on the prims, will have a direct mapping to a maya transform. This allows use of the
///             standard Maya scale/translate/rotate manipulators to control maya nodes directly.
///         \li For every custom plugin prim found, that node will need to be translated into Maya, and then connected
///             up with the other imported prims.
///         \li A certain amount of book keeping information will need to be generated, so that variant switching can
...
public:
  ...
  /// \brief  called after a proxy shape has been created. Traverses the prim hierarchy and looks to see whether
  ///         any custom plugin translators need to be called, and performs additional book-keeping.
  ...
  /// \brief  given a specific proxy shape, and a collection of UsdPrims that represent custom DagNode types, this
  ///         will generate a transform hierarchy that will allow you to map the UsdPrims to equivalent maya transforms
