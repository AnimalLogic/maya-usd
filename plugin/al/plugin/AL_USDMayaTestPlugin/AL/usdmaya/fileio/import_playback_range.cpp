    ...
    const char* const timeRangeOption)
{
  ...
  std::string usdFileName(std::string("AL_USDMayaTests_import_playback_range_") + testName + ".usda");
...
{
  MString importCmd = setupScene(
      g_staticUsd, "static_scene", "Import_Animations=1;", "override");
...
{
  MString importCmd = setupScene(g_animatedUsd, "no_animation", "Import_Animations=0;", "override");
  ASSERT_TRUE(MS::kSuccess == MGlobal::executeCommand(importCmd, true));
...
{
  MString importCmd = setupScene(
      g_animatedUsd, "unchanged_range", "Import_Animations=1;", nullptr);
  ASSERT_TRUE(MS::kSuccess == MGlobal::executeCommand(importCmd, true));
  // expect to match the min time from scene - no changes
  EXPECT_EQ(MAnimControl::minTime(), MTime(10.0));
  // expect to match the max time from scene - no changes
...
{
  MString importCmd = setupScene(
      g_animatedUsd, "override_animation_range", "Import_Animations=1;", "override");
  ASSERT_TRUE(MS::kSuccess == MGlobal::executeCommand(importCmd, true));
...
{
  MString importCmd = setupScene(
      g_animatedUsd, "combine_animation_range", "Import_Animations=1;", "combine");
  ASSERT_TRUE(MS::kSuccess == MGlobal::executeCommand(importCmd, true));
  // expect to match the min time from USD
  EXPECT_EQ(MAnimControl::minTime(), MTime(1.0));
